...
## PO Token / SABR — investigated, DOES NOT work on desktop (do not retry)

Some videos (commercial kids content, e.g. id `5wF1G3ft-0U`) cap at 360p: `web`/`mweb` return storyboards-only, `tv` says "DRM protected", `android` gives one 360p legacy stream (format 18). The user pointed out YTDLnis (Android) downloads them at 1080p. We investigated this **exhaustively** (decompiled YTDLnis APK → `assets/po_token.html` = BotGuard; tried both the jim60105 Rust `bgutil-pot.exe` and a locally-built Brainicism Node `bgutil` server v1.3.1):

- It is **NOT Widevine** — it's YouTube's PO-Token/SABR attestation gate.
- The PO Token mechanism works (token generated + "Retrieved a gvs PO Token" logged), BUT with yt-dlp 2026.03.17 the `web` client returns **only 4 storyboards (mhtml, zero video/audio) even for a NORMAL video** (`dQw4w9WgXcQ`) with a valid token. YouTube serves these via **SABR**, which yt-dlp can't yet download through the web client.
- YTDLnis succeeds because it runs on a real **Android device** → the `android` client gets full formats via genuine device attestation (DroidGuard), which **cannot be reproduced on Windows desktop**.

**Conclusion:** full quality for these restricted videos is not achievable on desktop with current yt-dlp. All PO-token code/assets were REMOVED. `_build_opts` uses `player_client=['default','android']` (4K/8K for normal videos, 360p + a note for restricted ones). Revisit only if yt-dlp gains SABR-download support for the web client.
